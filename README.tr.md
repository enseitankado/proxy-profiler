...
`ACCESS` hücresinde `mitm` görürsen, bu access probe sırasında TLS handshake'in
zincirde **self-signed bir cert** bulduğu için doğrulamayı reddetmesi demek.
Yani proxy isteğine gerçek `cloudflare.com` cert'ini değil, **kendi ürettiği**
bir cert'i geri veriyor — ders kitabı MITM imzası. Hem `_access_check_one`
(`proxyprof.py:941`) hem `_https_probe` aynı sinyalden `mitm_suspected`'ı
tetikliyor; iki bağımsız yol aynı bulguya varıyor → "tesadüf" şansı yok.
`--debug log.jsonl` ile çalıştırırsan ham exception mesajını görebilirsin:
`[SSL: CERTIFICATE_VERIFY_FAILED] ... self-signed certificate in certificate chain`.

Pattern olarak gözlenenler:
- Bu IP'lerin neredeyse tamamı `:4145` portunda (SOCKS4 daemon konvansiyon
  portu), çoğu `67.x / 68.71.x / 70.166.x / 72.x / 74.x / 98.x` bloklarında
  — public-listelerde dolaşan ve HTTPS'i decrypt edip ya credential skim ya
  reklam injection için **kasıtlı kurulmuş** "honeypot" proxy'leri.
- `to` / `err` / `?` kodları MITM **değildir** — sırasıyla timeout, sıradan
  ağ hatası (ConnectionReset / ServerDisconnected / payload), beklenmedik
  istisna. `mitm` SADECE cert-fail için işaretlenir.

SOCKS4 listelerinde %20-30 oranında mitm çıkması olağan; oran **sektörel**
(`:4145` honeypot kümesi access listesini ezdiği için), proxyprof'ta bir
false-positive değil. `--no-mitm-test` ile bu proxy'leri yine de çıktıya
almak istersen filtre kapatılır ama tabloda işaret kalır.
