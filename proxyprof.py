...
# Bootstrap fazı için minimal renksiz shim. Gerçek `_paint` ve renk
# sabitleri dosyanın çok altında tanımlanır (3900+); ama bağımlılık
# yükleme rutini bunlardan önce çalışıyor. Aynı isimle çağrılabilen sade
# bir versiyon koy — sonradan gelen renkli tanımlar bu shim'i shadow eder.
_C_DIM = _C_RED = _C_GREEN = _C_YELLOW = _C_CYAN = _C_BLUE = _C_MAGENTA = ""
_C_BOLD = _C_RESET = ""
...
def _prompt_choice(question: str, choices: list[str], default: str) -> str:
    """Numara ile seçenek soran prompt. Geçersiz cevap → default.

    choices: kabul edilecek değerler (örn. ["1","2","3"]). default: enter'a
    basılırsa dönen değer. Boş input / EOF / KeyboardInterrupt durumunda
    da default döner.
    """
    suffix = f"[{'/'.join(choices)}]"
    try:
        ans = input(f"{question} {suffix} ({default}): ").strip()
    except (EOFError, KeyboardInterrupt):
        sys.stderr.write("\n")
        return default
    if not ans:
...
def _print_manual_setup_instructions() -> None:
    """Kullanıcının kendi kuracağı senaryo için adım adım talimat.

    Sistem paketlerini kirletmek istemeyen ve `pip install --user`'a da
    güvenmek istemeyen kullanıcılar için canonical venv akışı. Tüm path'ler
    absolute — kullanıcı kopyala-yapıştır yapabilsin.
    """
    here = Path(__file__).resolve().parent
    pkg_str = " ".join(pkg for _, pkg in _REQUIRED)
    sys.stderr.write("\n" + t("deps.manual_intro") + "\n\n")
    sys.stderr.write(f"  $ cd {here}\n")
    sys.stderr.write(f"  $ python3 -m venv .venv\n")
    sys.stderr.write(f"  $ source .venv/bin/activate\n")
    sys.stderr.write(f"  $ pip install {pkg_str}\n")
    sys.stderr.write(f"  $ python3 proxyprof.py --help\n\n")
    sys.stderr.write(t("deps.manual_outro") + "\n\n")


def _install_command_launcher() -> int:
    """`proxy-prof` sarmalayıcı script'i PATH üzerinde bir dizine kur.

    Root ise /usr/local/bin'e, değilse ~/.local/bin'e yazar. Sarmalayıcı
    önce yerel `.venv/bin/python`'u dener (varsa o ile çalıştırır), yoksa
    sistem `python3`'üne düşer. Tekrar çağırıldığında üzerine yazar
    (idempotent — kullanıcı path değiştirdiyse veya repo'yu taşıdıysa
    güncellemek için).

    Bootstrap fazında çağrılır — `_paint` ve renk sabitleri henüz tanımlı
    değil; sade `proxyprof:` prefix'i kullanır.
    """
    prefix = "proxyprof:"
    here = Path(__file__).resolve().parent
    script = here / "proxyprof.py"
    if not script.exists():
        sys.stderr.write(f"{prefix} {t('launcher.script_missing', path=script)}\n")
        return 1
    ...
    target = target_dir / "proxy-prof"

    try:
        target_dir.mkdir(parents=True, exist_ok=True)
    except OSError as e:
        sys.stderr.write(
            f"{prefix} {t('launcher.mkdir_failed', dir=target_dir, err=e)}\n"
        )
        return 1

    wrapper = (
        "#!/bin/sh\n"
        "# proxy-prof launcher — auto-generated by `proxyprof --install-command`.\n"
        "# .venv varsa onu, yoksa sistem python3'ünü kullanır.\n"
        f"HERE={str(here)!r}\n"
        'if [ -x "$HERE/.venv/bin/python" ]; then\n'
        '    exec "$HERE/.venv/bin/python" "$HERE/proxyprof.py" "$@"\n'
        "else\n"
        '    exec python3 "$HERE/proxyprof.py" "$@"\n'
        "fi\n"
    )
    try:
        ...
        sys.stderr.write(
            f"{prefix} {t('launcher.write_failed', path=target, err=e)}\n"
        )
        return 1

    sys.stderr.write(f"{prefix} {t('launcher.installed', path=target)}\n")

    # PATH kontrolü: ~/.local/bin sistem PATH'inde olmayabilir; uyaralım.
    path_dirs = os.environ.get("PATH", "").split(os.pathsep)
    if str(target_dir) not in path_dirs:
        sys.stderr.write(
            f"{prefix} {t('launcher.path_warning', dir=target_dir)}\n"
        )
    else:
        sys.stderr.write(f"{prefix} {t('launcher.usage_hint')}\n")
    return 0


def _pre_parse_bootstrap_flags(argv: list[str]) -> None:
    """`--setup-help` ve `--install-command` flag'lerini bağımlılık
    yüklemeden ÖNCE yakala.

    İkisi de hiçbir bağımlılığa ihtiyaç duymaz; eksik aiohttp ortamında
    bile çalışmalı (kullanıcı zaten bu durumda yardım almak için
    çağırıyor). argparse spec'inde de gözükürler ama gerçek handler
    burası — argparse'a düşmeden exit eder.
    """
    if "--setup-help" in argv:
        _print_manual_setup_instructions()
        sys.exit(0)
    if "--install-command" in argv:
        sys.exit(_install_command_launcher())


def _ensure_deps() -> None:
    """Eksik bağımlılıkları interaktif 3-yollu menü ile hallet.
    ...
    3. TTY ise üç seçenek sun:
       a) Sistem'e yükle (pip install — root değilse `--user` ile, in-venv
          ise sade)
       b) Yerel `.venv` oluştur ve oraya yükle (sudo yok, sistem temiz kalır)
       c) Manuel — adım adım talimatı göster, çık
       Her dalın sonunda süreç `os.execv` ile temiz biçimde yeniden başlar
       (manuel hariç — orada kullanıcı kendi çalıştırır).
    """
    ...
    # Üç-yollu menü.
    sys.stderr.write(
        f"\n{_paint('proxyprof:', _C_DIM)} {t('deps.missing', pkgs=pkg_str)}\n\n"
    )
    sys.stderr.write(t("deps.menu_intro") + "\n\n")
    if in_venv:
        sys.stderr.write(
            f"  1) {t('deps.menu_system_in_venv', prefix=sys.prefix)}\n"
        )
    elif is_root:
        sys.stderr.write(f"  1) {t('deps.menu_system_root')}\n")
    else:
        sys.stderr.write(f"  1) {t('deps.menu_system_user')}\n")
    venv_dir = Path(__file__).resolve().parent / ".venv"
    sys.stderr.write(f"  2) {t('deps.menu_venv', dir=venv_dir)}\n")
    sys.stderr.write(f"  3) {t('deps.menu_manual')}\n\n")
    choice = _prompt_choice(
        t("deps.menu_prompt"), choices=["1", "2", "3"], default="2",
    )
    sys.stderr.write("\n")
    ...
    if choice == "1":
        # Sistem'e yükle. in-venv ise sade pip; değilse --user (sudo yok,
        # PEP 668 ile uyumlu). Root ise --user'ı atla (root'un home'u
        # tipik kullanım değil).
        cmd = [sys.executable, "-m", "pip", "install"]
        if not in_venv and not is_root:
            cmd.append("--user")
        cmd.extend(missing)
        sys.stderr.write(
            f"{_paint('proxyprof:', _C_DIM)} {t('deps.running', cmd=' '.join(cmd))}\n"
        )
        rc = subprocess.run(cmd).returncode
        if rc != 0:
            sys.stderr.write(
                f"{_paint('proxyprof:', _C_DIM)} "
                f"{t('deps.system_install_failed')}\n"
            )
            sys.exit(1)
        os.execv(sys.executable, [sys.executable, *sys.argv])

    # choice == "2" → yerel venv. Default ve önerilen yol.
    sys.stderr.write(
        f"{_paint('proxyprof:', _C_DIM)} "
...
_pre_parse_bootstrap_flags(sys.argv[1:])
_ensure_deps()
...
def main(argv: list[str] | None = None) -> int:
    ...
    # Aşağıdaki iki flag aslında `_pre_parse_bootstrap_flags` tarafından
    # bağımlılık check'inden ÖNCE yakalanır ve burada hiç args'a düşmez —
    # ama --help çıktısında listelensinler ve `proxyprof --help` ile
    # öğrenilebilsinler diye argparse'a tanıtıyoruz.
    g_misc.add_argument(
        "--setup-help", action="store_true", dest="setup_help",
        help=t("cli.help.setup_help"),
    )
    g_misc.add_argument(
        "--install-command", action="store_true", dest="install_command",
        help=t("cli.help.install_command"),
    )
